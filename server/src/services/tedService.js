...
export function detectPortalName(url) {
  ...
  if (u.includes('verama.com') || u.includes('eworkgroup.com')) return 'Verama';
...
export function buildExpertQuery(filters = {}) {
  ...
  // Excluded keywords (Free text search, negated)
  if (filters.excludeKeywords) {
    const exClause = parseTextFieldQuery('FT', filters.excludeKeywords);
    if (exClause) {
      parts.push(`NOT ${exClause}`);
    }
  }
